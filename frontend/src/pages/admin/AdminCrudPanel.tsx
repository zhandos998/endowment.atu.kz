...
type RowValue = boolean | string | number | DocumentLink[] | null | undefined;
...
function normalizeCheckboxValue(value: RowValue) {
    ...
    return (
      <div className="grid gap-1">
        {value.map((item, index) => (
          <a
            key={`${item.url ?? item.name ?? 'file'}-${index}`}
            href={item.url ?? '#'}
            target="_blank"
            rel="noreferrer"
            className="truncate text-xs font-semibold text-accent hover:text-navy"
          >
            {item.name ?? `Документ ${index + 1}`}
          </a>
        ))}
      </div>
    );
...
export default function AdminCrudPanel({ config }: { config: EntityConfig }) {
  ...
  const [manualSlugFields, setManualSlugFields] = useState<Record<string, boolean>>({});
  ...
  const modalWidthClass = config.formSize === 'wide' ? 'max-w-4xl' : 'max-w-2xl';

  const load = useCallback(async () => {
    const { data } = await api.get(`/admin/${config.endpoint}`, {
      params: { search, page, per_page: 8, ...config.requestParams },
    });
    setRows(data.data);
    setMeta(data.meta);
  }, [config.endpoint, config.requestParams, page, search]);
  ...
  function openCreate() {
    ...
    config.fields.forEach((field) => {
      if (field.type === 'checkbox') {
        defaults[field.name] = field.defaultChecked ?? true;
      }
    });
  ...
  function openEdit(row: Row) {
    ...
    config.fields.forEach((field) => {
      ...
      if (field.type === 'checkbox') {
        values[field.name] = normalizeCheckboxValue(row[field.name]);
        return;
      }

      const raw = row[field.name];
      values[field.name] = raw == null || Array.isArray(raw) ? '' : String(raw).slice(0, field.type === 'datetime-local' ? 16 : undefined);
    });
  ...
  function updateField(fieldName: string, value: FormValue) {
    setFormValues((current) => {
      ...
      if (!editing) {
        config.fields
          .filter((field) => field.autoSlugFrom === fieldName)
          .forEach((field) => {
            if (!manualSlugFields[field.name]) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    Object.entries(config.hiddenPayload ?? {}).forEach(([key, value]) => {
    ...
    <div>
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">{config.description ?? 'Редактирование контента, фильтр, пагинация и загрузка изображений.'}</p>
        </div>
        {canCreate && (
      ...
      <div className="mb-5 flex gap-3 rounded-xl bg-white p-4 shadow-soft">
        ...
        <button onClick={() => setPage(1)} className="rounded-lg bg-navy px-5 text-sm font-bold text-white">
          Найти
        </button>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((row) => (
                <tr key={String(row.id)} className="hover:bg-slate-50/70">
                  {config.tableFields.map((field) => (
                    <td key={field} className="max-w-[320px] truncate px-5 py-4 text-slate-700">
        ...
        {meta && (
          <div className="flex items-center justify-between border-t border-slate-100 px-5 py-4 text-sm text-slate-500">
            ...
            <div className="flex items-center gap-2">
              <button disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))} className="rounded-lg border border-slate-200 px-3 py-2 disabled:opacity-40">
                Назад
              </button>
              <span>
                {meta.current_page} / {meta.last_page}
              </span>
              <button disabled={page >= meta.last_page} onClick={() => setPage((value) => value + 1)} className="rounded-lg border border-slate-200 px-3 py-2 disabled:opacity-40">
                Вперед
              </button>
            </div>
          </div>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-navy/55 p-4 backdrop-blur-sm">
          <form onSubmit={submit} className={`max-h-[90vh] w-full ${modalWidthClass} overflow-auto rounded-xl bg-white p-6 shadow-premium`}>
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-2xl font-semibold text-navy">
                {editing ? 'Редактировать' : 'Добавить'}: {title}
              </h2>
            ...
            <div className="grid gap-5">
              {config.fields.map((field) =>
                field.type === 'richtext' ? (
                  <div key={field.name} className="block text-sm font-semibold text-slate-700">
                    <div>{field.label}</div>
                    <AdminRichTextEditor value={String(formValues[field.name] ?? '')} required={field.required} onChange={(value) => updateField(field.name, value)} />
                  </div>
                ) : (
                  <label key={field.name} className="block text-sm font-semibold text-slate-700">
                    ...
                    <>
                      ...
                      />
                      {typeof editing?.[`${field.name}_url`] === 'string' && (
                        <a href={String(editing[`${field.name}_url`])} target="_blank" rel="noreferrer" className="mt-2 inline-flex text-xs font-bold text-accent hover:text-navy">
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        if (field.autoSlugFrom) {
                          setManualSlugFields((value) => ({ ...value, [field.name]: true }));
                        }
            ...
            <button className="mt-7 w-full rounded-full bg-accent px-7 py-4 text-sm font-bold text-white hover:bg-[#ef6f2d]">Сохранить</button>
          </form>
        </div>
      )}
    </div>
